...
import $file.dependencies.utility.build
...
object mydiplomacy extends dependencies.diplomacy.build.diplomacy {
  override def scalaVersion = sv
  override def millSourcePath = os.pwd /  "dependencies" / "diplomacy" / "diplomacy"
  def chisel3Module: Option[PublishModule] = Some(mychisel3)
}

object myutility extends dependencies.utility.build.utility {
  override def scalaVersion = sv
  override def millSourcePath = os.pwd /  "dependencies" / "utility" / "utility"
  def chisel3Module: Option[PublishModule] = Some(mychisel3)
}
...
object myrocketchip extends dependencies.`rocket-chip`.common.CommonRocketChip {
  ...
  def diplomacyModule: PublishModule = mydiplomacy
  def utilityModule: PublishModule = myutility
}


// For modules not support mill yet, need to have a ScalaModule depend on our own repositories.
// will be forked one by one.
